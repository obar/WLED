...
#ifndef USERMOD_PRESET_ARRAY_MAX_PRESETS
#define USERMOD_PRESET_ARRAY_MAX_PRESETS 64
#endif

#ifndef USERMOD_PRESET_ARRAY_MAX_BANKS
#define USERMOD_PRESET_ARRAY_MAX_BANKS 23
#endif

class UsermodPresetArray : public Usermod {
  private:
    bool enabled = false;
    uint8_t presets[USERMOD_PRESET_ARRAY_MAX_PRESETS];
    uint8_t bankOffset[USERMOD_PRESET_ARRAY_MAX_BANKS+1];
    String presetArray = "1";
    String bootupPresetArray;
  ...
  public:
    ...
    void readFromJsonState(JsonObject& root) override {
      String cmd;
      bool cmdExists = getJsonValue(root["pa"], cmd);
...
const char* UsermodPresetArray::sName = "Preset Array";
const char* UsermodPresetArray::sEnabled = "Enable";
const char* UsermodPresetArray::sArray = "Preset banks>"; // Trailing > makes this a textarea in settings

bool UsermodPresetArray::readArray() {
  int readX = 0;
  int i = 0;
  int cursor = 0;
  ...
  while (!finished) {
    ...
    if (readX >= USERMOD_PRESET_ARRAY_MAX_BANKS) {
      DEBUG_PRINTLN("Preset array: exceeded max banks");
      break;
    }
    ...
    while (!bankFinished) {
      ...
      if (i >= USERMOD_PRESET_ARRAY_MAX_PRESETS) {
        DEBUG_PRINTLN("Preset array: exceeded max presets");
        break;
      }
...
void UsermodPresetArray::printArray() {

}

void UsermodPresetArray::incBank(bool wrap){
  if (x == xMax && wrap) {
    x = 0; y = 0;
    applyPresetArray();
  } else if (x < xMax) {
...
void UsermodPresetArray::decBank(bool wrap){
  if (x == 0 && wrap) {
    x = xMax; y = 0;
    applyPresetArray();
  } else if (x > 0) {
...
void UsermodPresetArray::incPreset(bool wrap){
...
void UsermodPresetArray::decPreset(bool wrap){
  int yMax = bankOffset[x+1] - bankOffset[x] - 1;
  if (yMax == 0) {
    // We don't have anywhere to cycle
  } else if (y == 0 && wrap) {
